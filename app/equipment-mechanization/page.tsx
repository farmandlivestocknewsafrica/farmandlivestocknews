...
async function getCategoryData() {
  ...
  try {
    const articlesRes = await supabase
      .from('articles')
      .select('*')
      .eq('category', 'equipment_mechanisation')
    ...
    return { category: 'Equipment & Mechanization', articles: [] }
  }
}
            ...
            {injectAds(articles, { itemsPerAd: 4 }).map((item, index) => {
              if (isAdItem(item)) {
                return (
                  <AdCard
                    key={item.id}
                    title="Farm Equipment & Machinery Solutions"
                    description="Browse the latest mechanization tools and equipment for modern farming operations."
                  />
                )
              }
