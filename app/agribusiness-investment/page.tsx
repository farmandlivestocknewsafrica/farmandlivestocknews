...
async function getCategoryData() {
  ...
  try {
    const articlesRes = await supabase.from('articles').select('*').eq('category', 'agribusiness').order('published_at', { ascending: false })
    return { category: 'Agribusiness', articles: articlesRes.data || [] }
  } catch (error) {
    return { category: 'Agribusiness', articles: [] }
  }
}

export default async function AgribusinessPage() {
              ...
              if (isAdItem(item)) {
                return (
                  <AdCard
                    ...
                    description="Explore partnerships and funding solutions for agricultural enterprises across Africa."
                  />
                )
              }
